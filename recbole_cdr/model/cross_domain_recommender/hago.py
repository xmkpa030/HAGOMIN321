...
class HAGO(CrossDomainRecommender):
    ...
    def build_weighted_coord_connection_matrix(self, n_nodes):
        # dynamic weighted coordinator block:
        # W[a,b] = ReLU(cos(e_cu_a, e_ci_b)), only user-coordinator <-> item-coordinator.
        n_user_coods = self.num_graphs * self.num_coods
        coord_offset = self.total_num_users + self.total_num_items
        norm_coord_embedding = F.normalize(self.coord_embedding.weight, dim=-1)
        user_coord_embedding = norm_coord_embedding[:n_user_coods]
        item_coord_embedding = norm_coord_embedding[n_user_coods:]

        cross_scores = torch.relu(user_coord_embedding @ item_coord_embedding.transpose(0, 1))
        nonzero_index = torch.nonzero(cross_scores > 0, as_tuple=False)
        ...
        values = cross_scores[nonzero_index[:, 0], nonzero_index[:, 1]]
        row_forward = nonzero_index[:, 0] + coord_offset
        col_forward = nonzero_index[:, 1] + coord_offset + n_user_coods
        row_backward = col_forward
        col_backward = row_forward

        row = torch.cat([row_forward, row_backward], dim=0)
        col = torch.cat([col_forward, col_backward], dim=0)
        matrix_index = torch.stack([row, col], dim=0)
        matrix_values = torch.cat([values, values], dim=0)

        return torch.sparse_coo_tensor(matrix_index, matrix_values, torch.Size((n_nodes, n_nodes)))
    ...
    def get_norm_adj_mat_with_cood(self, interaction_matrix, n_users=None, n_items=None):
        ...
        weighted_coord_block = self.build_weighted_coord_connection_matrix(n_nodes)
            
        return self.SparseL+coord_matrix+coord_matrix.T+weighted_coord_block
