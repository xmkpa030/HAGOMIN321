...
class PretrainDataLoader(AbstractDataLoader):
    def __init__(self, config, dataset, sampler, shuffle=False):
        ...
        super().__init__(config, dataset, sampler, shuffle=shuffle)
        self.pretrain_method = config['pretrain_method']
        source_node_list = list(range(dataset.num_overlap_user))+list(range(dataset.num_overlap_user+dataset.num_target_only_user, dataset.num_total_user)) + list(range(dataset.num_total_user, dataset.num_total_user+dataset.num_overlap_item))+list(range(dataset.num_total_user+dataset.num_overlap_item+dataset.num_target_only_item, dataset.num_total_user+dataset.num_total_item))
        self.source_node_num = len(source_node_list)
        all_random_node_list = torch.tensor(source_node_list)[torch.randperm(self.source_node_num)]
        selected_node_num_for_random_walk = config['sampling_num']
        self.random_node_list = all_random_node_list[:selected_node_num_for_random_walk]
        self.sample_size = len(self.random_node_list)
    ...
    def collate_fn(self, index):
        ...
        data = self.random_node_list[index]
        return data
    
    def get_model(self, model):
        self.model = model
        self.graph_sampleing()
        
    def graph_sampleing(self):
        ...
        A_coo = sp.coo_matrix(self.model.A)
        walk_list = random_walk(torch.LongTensor(A_coo.row), torch.LongTensor(A_coo.col), self.random_node_list, walk_length=80, p=4, q=4)

        for g in tqdm.tqdm(walk_list):
            unique_g = torch.tensor(list(Counter(g.tolist()).keys()))
            
            g = torch.cat([unique_g[:50], torch.arange(self.model.A.shape[0]-self.model.total_num_coods, self.model.A.shape[0])])

            
            if self.pretrain_method == 'graphcl':
                augs, aug_ratio = random.choices(['dropN', 'permE', 'maskN'], k=2), random.randint(1, 3) * 1.0 / 10
            else:
                augs, aug_ratio = "none", 0
            view_g, walk = graph_views(adj_matrix=self.model.A, walk=g, aug=augs[0], aug_ratio=aug_ratio)
            self.sub_adj_list_1.append(view_g)
            self.walk_list_1.append(walk)
            view_g, walk = graph_views(adj_matrix=self.model.A, walk=g, aug=augs[1], aug_ratio=aug_ratio)
            self.sub_adj_list_2.append(view_g)
            self.walk_list_2.append(walk)
    ...
    def _next_batch_data(self):
        self.pr += self.step
        return self.sub_adj_list_1[self.pr:self.pr + self.step], self.walk_list_1[self.pr:self.pr + self.step], self.sub_adj_list_2[self.pr:self.pr + self.step], self.walk_list_2[self.pr:self.pr + self.step]
