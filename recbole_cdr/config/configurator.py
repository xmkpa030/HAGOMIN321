...
class CDRConfig(Config):
    ...
    def __init__(self, model=None, config_file_list=None, config_dict=None, gpu_id=None):
        """
        Args:
        ...
        self.final_config_dict['model_name'] = model

        if gpu_id is not None:
            self.final_config_dict['gpu_id'] = gpu_id
    ...
    def _check_cross_domain(self):
        ...
        if source_dataset_name == 'ml-100k' or source_dataset_name == 'ml-1m':
            ...
            if type(source_dataset_name) == list:
                self.final_config_dict['source_domain']['data_path'] = [os.path.join(data_path, dataset) for dataset in source_dataset_name]
            else:
